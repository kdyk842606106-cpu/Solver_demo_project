...
INPUT_ROLES = {"input", "context_input"}
OUTPUT_ROLES = {"output", "declared_output"}
...
def _project_solver_ready_edges(
    ...
    *,
    ...
    for atomic in context.atomic_activities:
        if not include_inactive and not atomic.is_active:
            continue
        activity_id = _atomic_graph_id(atomic.id)
        for binding in _context_bindings_for_atomic(
            atomic.id,
            context,
            include_inactive=include_inactive,
        ):
            if not include_inactive and not binding.is_active:
                continue
            coverage_status = _coverage_status(binding, state_by_id, state_children)
            for leaf_id in binding.covered_leaf_state_ids or []:
                leaf = state_by_id.get(int(leaf_id))
                ...
                edges.append(
                    _solver_ready_leaf_edge(
                        ...
                        activity_id=activity_id,
                        role="context_input",
                        source_kind="inherited_context_binding",
                        coverage_status=coverage_status,
                    )
                )
...
def _add_issue_counts_to_summary(
    ...
    graph.setdefault("summary", {}).update({
        ...
        "partial_virtual_activity_count": sum(
            1 for issue in modeling_issues if issue["code"] == "VIRTUAL_OUTPUT_NOT_IMPLEMENTED"
        ),
    })
...
def _build_graph_from_context(context: NetworkEditorContext, payload: NetworkEditorRequest) -> dict[str, Any]:
    ...
    if not payload.activity_scope_node_ids and payload.activity_depth <= 0:
        ...
        selected_atomic_ids = {
            ref.atomic_activity_id
            for ref in context.package_refs
            if ref.activity_node_id in selected_activity_depths
            and selected_activity_depths[ref.activity_node_id] + 1 <= payload.activity_depth
            and (payload.include_inactive or ref.is_active)
        }
...
def _validate_projected_graph(
    ...
    *,
    ...
    output_leaf_ids_by_activity_graph_id: dict[str, set[int]] = defaultdict(set)
    for binding in graph["bindings"]:
        if binding["binding_role"] == "output":
            ...
            output_leaf_ids_by_activity_graph_id[activity_id].update(binding["covered_leaf_state_ids"] or [])

    for binding in graph["bindings"]:
        if binding["binding_role"] != "declared_output" or not binding.get("activity_node_id"):
            continue
        descendant_graph_ids = _activity_descendant_graph_ids(
        ...
        for activity_id in descendant_graph_ids:
            implemented.update(output_leaf_ids_by_activity_graph_id.get(activity_id, set()))
        declared = {int(item) for item in binding["covered_leaf_state_ids"] or []}
        missing = sorted(declared - implemented)
        if missing:
            issue = _make_issue(
                "VIRTUAL_OUTPUT_NOT_IMPLEMENTED",
                "warning",
                "implementation",
                "Virtual activity declared output is not fully implemented by child executable outputs",
                related_state_ids=missing,
                related_activity_ids=[_activity_graph_id(binding["activity_node_id"])],
                details={"binding_id": binding["id"], "missing_leaf_state_ids": missing},
                suggested_action="Add executable output bindings under this virtual activity.",
            )
            modeling_issues.append(issue)
            solver_ready_issues.append({**issue, "severity": "error"})
...
async def analyze_network_editor_impact(
    ...
    inherited_state_ids = _edge_state_ids(
        graph["edges"],
        activity_graph_id=activity_graph_id,
        edge_type="STATE_TO_ACTIVITY",
        roles={"context_input"},
    ...
    output_state_ids = _edge_state_ids(
        ...
        activity_graph_id=activity_graph_id,
        edge_type="ACTIVITY_TO_STATE",
        roles={"output", "declared_output"},
    )

    if activity.get("atomic_activity_id") is not None:
        inherited_leaf_ids: list[int] = []
        for binding in _context_bindings_for_atomic(
            activity["atomic_activity_id"],
            context,
            include_inactive=payload.include_inactive,
        ):
            inherited_leaf_ids.extend(
                int(item)
                for item in binding.covered_leaf_state_ids or []
                if payload.include_inactive
                or (
                    state_by_id.get(int(item)) is not None
                    and state_by_id[int(item)].is_active
                )
            )
        inherited_state_ids.extend(_state_graph_id(state_id) for state_id in inherited_leaf_ids)
...
def _context_bindings_for_atomic(
    atomic_activity_id: int,
    context: NetworkEditorContext,
    *,
    include_inactive: bool,
) -> list[ActivityStateBinding]:
    activity_children = _children_by_parent(context.activity_nodes)
    refs = _activity_package_refs_by_atomic(context).get(atomic_activity_id, [])
    ...
    for ref in refs:
        ...
        current = activity_by_id.get(ref.activity_node_id)
        while current is not None:
            if include_inactive or current.is_active:
                activity_ids.add(current.id)
            current = activity_by_id.get(current.parent_id) if current.parent_id is not None else None
    for node in context.activity_nodes:
        if node.level == 3 and node.id in activity_ids and (include_inactive or node.is_active):
            for descendant_id in _activity_descendant_ids(node.id, activity_children):
                descendant = activity_by_id.get(descendant_id)
                if include_inactive or (descendant is not None and descendant.is_active):
                    activity_ids.add(descendant_id)
    return [
        binding for binding in context.bindings
        if binding.activity_node_id in activity_ids
        and binding.binding_role == "context_input"
        and (include_inactive or binding.is_active)
    ]
