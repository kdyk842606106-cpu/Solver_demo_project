...
async def expand_layered_context(
    ...
    atomic_refs_by_atomic: dict[int, list[ActivityPackageAtomicRef]] = defaultdict(list)
    for ref in atomic_refs:
        atomic_refs_by_package[ref.activity_node_id].append(ref)
        atomic_refs_by_atomic[ref.atomic_activity_id].append(ref)
    for refs in atomic_refs_by_package.values():
        refs.sort(key=lambda item: (item.sort_order, item.id))
    for refs in atomic_refs_by_atomic.values():
        refs.sort(key=lambda item: (item.sort_order, item.id))
    ...
    def is_activity_node_in_active_path(node: ActivityNode) -> bool:
    ...
    if not payload.activity_scope_node_ids:
        active_packages = [
            node
            for node in activity_nodes
            if node.level == 2 and is_activity_node_in_active_path(node)
        ]
        for package in sorted(active_packages, key=lambda item: (item.sort_order, item.id)):
            package_path = _path_for_node(package, activity_by_id)
            inherited_preconditions = build_inherited_preconditions(package_path)
            for ref in [
                item
                for item in atomic_refs_by_package.get(package.id, [])
                if payload.include_inactive or item.is_active
            ]:
                append_atomic_candidate(
                    atomic=ref.atomic_activity,
                    source_activity_node_id=package.id,
                    package_ref_id=ref.id,
                    source_path=package_path,
                    inherited_preconditions=inherited_preconditions,
                    missing_ref_id=ref.id,
                )

        for atomic in sorted(atomic_activities, key=lambda item: (item.sort_order, item.id)):
            if atomic_refs_by_atomic.get(atomic.id) and ("atomic", atomic.id) in candidate_keys:
                continue
            append_atomic_candidate(
                atomic=atomic,
