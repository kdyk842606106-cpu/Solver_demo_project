...
def _activity_group_windows(model: ScheduleModel) -> dict[int, dict[str, Any]]:
    cached = model.objective_cache.get("activity_group_windows")
    if cached is not None:
        return cached

    windows: dict[int, dict[str, Any]] = {}
    for group_id, step_orders in sorted(model.activity_groups.items()):
        ...
        group_start = model.model.new_int_var(0, model.horizon, f"group_{group_id}_start")
        group_end = model.model.new_int_var(0, model.horizon, f"group_{group_id}_end")
        group_span = model.model.new_int_var(0, model.horizon, f"group_{group_id}_span")
    ...
    model.objective_cache["activity_group_windows"] = windows
    return windows


def _activity_group_gap_terms(model: ScheduleModel) -> list[Any]:
    cached = model.objective_cache.get("activity_group_gaps")
    if cached is not None:
        return cached

    terms = []
    for group_id, window in _activity_group_windows(model).items():
        duration_sum = sum(model.task_vars[step_order].duration for step_order in window["step_orders"])
        gap = model.model.new_int_var(0, model.horizon, f"group_{group_id}_gap")
        model.model.add(gap >= window["span"] - duration_sum)
        terms.append(gap)
    model.objective_cache["activity_group_gaps"] = terms
    return terms


def _activity_group_interruption_terms(model: ScheduleModel) -> list[Any]:
    cached = model.objective_cache.get("activity_group_interruptions")
    if cached is not None:
        return cached
    ...
    for group_id, window in _activity_group_windows(model).items():
        ...
        for outside_step in outside_steps:
            ...
            starts_inside = model.model.new_bool_var(f"group_{group_id}_outside_{outside_step}_starts_inside")
            ends_inside = model.model.new_bool_var(f"group_{group_id}_outside_{outside_step}_ends_inside")
            interruption = model.model.new_bool_var(f"group_{group_id}_outside_{outside_step}_interrupts")
    ...
    model.objective_cache["activity_group_interruptions"] = terms
    return terms
