...
def split_sql_statements(sql_content: str) -> list[str]:
    ...
    return [statement for statement in statements if not is_transaction_control_statement(statement)]


def is_transaction_control_statement(statement: str) -> bool:
    """Ignore seed-owned transaction markers; the loader owns the transaction."""
    meaningful_lines = [
        line.strip()
        for line in statement.splitlines()
        if line.strip() and not line.strip().startswith("--")
    ]
    normalized = " ".join(meaningful_lines).upper()
    return normalized in {"BEGIN", "START TRANSACTION", "COMMIT", "ROLLBACK"}
    ...
    async with async_engine.connect() as conn:
        ...
        try:
            ...
            for i, statement in enumerate(statements, 1):
                ...
                try:
                    await conn.exec_driver_sql(statement_to_execute)
