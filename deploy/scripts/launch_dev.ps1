...
function Ensure-WorkspacePostgres {
    $dbHost = Get-EnvFileValue $envFile 'DB_HOST'
    ...
    if (-not $usesLocalHost -or -not (Test-Path -LiteralPath $localPgData)) {
        return
    }
    ...
    Write-Host ("Starting workspace PostgreSQL on DB_PORT={0}" -f $dbPort)
    Invoke-Native $pgCtl @('-D', $localPgData, '-l', $localPgLog, '-o', ("-p {0}" -f $dbPort), 'start')
}
...
Write-Section 'Loading project environment'
