...
function foldedParentsGraphResponseForRequest(request: any) {
  ...
  const stateExpanded = stateRoots.has(1) && Number(body.state_depth) === 0
  const activityExpanded = activityScopes.has(10) && Number(body.activity_depth) === 0
...
function wrappedFlowGraphResponseForRequest(request: any) {
  const body = JSON.parse(request.postData() || '{}')
  const stateRoots = new Set((body.state_root_ids || []).map((id: any) => Number(id)))
  const state4Expanded = stateRoots.has(4) && Number(body.state_depth) === 0
...
function collapsedProxyEdgeGraphResponseForRequest(request: any) {
  ...
  const activityExpanded = activityScopeIds.has(10) && Number(body.activity_depth) === 0
...
function overlappingExpandedContainersGraphResponseForRequest(request: any) {
  ...
  const stateRoots = new Set((body.state_root_ids || []).map((id: any) => Number(id)))
  const activityScopes = new Set((body.activity_scope_node_ids || []).map((id: any) => Number(id)))
  const stateExpanded = Number(body.state_depth) === 0
  const activityExpanded = activityScopes.size > 0 && Number(body.activity_depth) === 0
  const state1 = graphResponse.state_nodes.find((node: any) => node.state_node_id === 1)
  ...
  return {
    ...
    activity_nodes: [
      ...
      ...(activityExpanded ? [{
        ...atomic20,
        metadata_json: { _network_editor_layout: { x: 520, y: 238 }, instance_note: 'package ref layout' },
      }] : []),
      virtual11,
      ...(activityExpanded ? [atomic21InPackage] : []),
    ].filter(Boolean),
    edges: [],
    summary: {
      ...graphResponse.summary,
      state_instance_count: stateExpanded ? 4 : 2,
      activity_node_count: activityExpanded ? 4 : 2,
      edge_count: 0,
    },
  }
}
  ...
  await page.route(/\/(health|api\/v1\/)/, async (route: any, request: any) => {
    ...
    if (url.endsWith('/api/v1/activity-nodes/10/atomic-activity-refs')) {
      const response = typeof options.activityPackageRefs === 'function'
        ...
        : (options.activityPackageRefs || activityPackageAtomicRefs)
      await route.fulfill({
        status: 200,
...
async function pressActionAndWaitForGraphReloadWithoutTemporaryConnection(page: any, locator: any) {
  await Promise.all([
    ...
    pressActionWithoutTemporaryConnection(page, locator),
  ])
  await waitForNetworkEditorIdle(page)
}
...
test.describe('Network Editor state-transition MVP', () => {
  ...
  test('projects referenced atomic library states on first load with transition details', async ({ page }) => {
  ...
  test('shows flow backbone and active paths without creating drafts', async ({ page }) => {
    ...
    await expect.poll(() => page.locator('[data-flow-role="precondition"][data-flow-state="backbone"]').count()).toBeGreaterThan(0)
    await expect.poll(() => page.locator('[data-flow-role="realizer"][data-flow-state="backbone"]').count()).toBeGreaterThan(0)
    await expect.poll(() => page.locator('[data-flow-role="precondition"][data-flow-route="short"]').count()).toBeGreaterThan(0)
    await expect(page.locator('[data-flow-role="precondition"][data-flow-route="short"]').first()).toHaveAttribute('data-source-side', 'right')
    await expect(page.locator('[data-flow-role="precondition"][data-flow-route="short"]').first()).toHaveAttribute('data-target-side', 'left')
    await expect(page.getByTestId('network-editor-activity-node-atomic_activity:20')).toHaveCount(0)
    ...
    await page.mouse.move(4, 4)
    await expect.poll(() => page.locator('[data-flow-state="active"]').count()).toBe(0)
    ...
    expect(relayPosition.x).toBeGreaterThan(inputStatePosition.x + 90)
  ...
  test('auto arrange alternates state columns with transition relay columns', async ({ page }) => {
    await openNetworkEditorFixture(page, { expectActivityNodes: false })
    ...
    expect(Math.abs(previewSourcePosition.x - sourcePosition.x)).toBeLessThan(2)
    expect(Math.abs(previewSourcePosition.y - sourcePosition.y)).toBeLessThan(2)
    expect(Math.abs(previewRelayPosition.x - relayPosition.x)).toBeLessThan(2)
    expect(Math.abs(previewRelayPosition.y - relayPosition.y)).toBeLessThan(2)
    expect(Math.abs(previewTargetPosition.x - targetPosition.x)).toBeLessThan(2)
    expect(Math.abs(previewTargetPosition.y - targetPosition.y)).toBeLessThan(2)
  })
  ...
  test('auto arrange lays out long state transition ranks left to right', async ({ page }) => {
    ...
    await clickAndWaitForGraphReload(page, () => packageNode.locator('[data-action="toggle"]').click())
...
test.describe('Network Editor — edit session and semantic labels', () => {
  ...
  test('keeps preview read-only until entering edit and renders semantic edge labels', async ({ page }) => {
    ...
    await expect(page.getByTestId('network-editor-create-activity').last()).toBeVisible()
  ...
  test('does not start connection drag from edit-mode action buttons and toggles packages', async ({ page }) => {
    ...
    await expect(page.locator('[data-cell-id="temporary-connection-preview"]')).toHaveCount(0)
    await expect(page.locator('[data-cell-id^="pending-binding-preview"]')).toHaveCount(0)
    await focusStrip.locator('.el-button').last().click()
    await expect(focusStrip).toBeHidden()

    const virtualContainer = page.getByTestId('network-editor-virtual-activity-container-activity_node:10')
    await expect(virtualContainer).toBeVisible()
    await pressActionAndWaitForGraphReloadWithoutTemporaryConnection(page, virtualActivity.locator('[data-action="toggle"]'))
    await expect(virtualContainer).toBeHidden()
    await pressActionAndWaitForGraphReloadWithoutTemporaryConnection(page, virtualActivity.locator('[data-action="toggle"]'))
    await expect(virtualContainer).toBeVisible()
    await expect(page.getByTestId('network-editor-activity-node-atomic_activity:20')).toBeVisible()
  ...
  test('shows collapsed proxy edges when child endpoints are folded', async ({ page }) => {
    ...
    await expect.poll(() => page.locator('[data-flow-role="proxy"][data-flow-route="short"]').count()).toBeGreaterThan(0)
    await expect(page.locator('[data-flow-role="proxy"][data-flow-route="short"]').first()).toHaveAttribute('data-source-side', 'right')
    await expect(page.locator('[data-flow-role="proxy"][data-flow-route="short"]').first()).toHaveAttribute('data-target-side', 'left')
  ...
  test('offers draft states to atomic activities and allows empty input/output states', async ({ page }) => {
    ...
    await expect(draftAtomicActivity.locator('.semantic-port')).toHaveCount(0)
    await expect(page.getByTestId('network-editor-state-node-draft-state:draft-1')).toHaveCount(0)
  ...
  test('renders draft virtual activities and folds draft child activities locally', async ({ page }) => {
    ...
    await openCreateMenuItem(page, 'network-editor-create-activity')
  ...
  test('keeps expanded containers from overlapping and restores folded positions', async ({ page }) => {
    ...
    expect(rectsOverlap(await locatorClientRect(prepContainer), await locatorClientRect(packContainer))).toBe(false)
    await expect(page.getByTestId('network-editor-submit-draft')).toBeDisabled()
  ...
  test('moves folded package descendants with the parent before expansion', async ({ page }) => {
    await openNetworkEditorFixture(page, {
      graphResponse: foldedParentsGraphResponseForRequest,
      expectInitialLeafNodes: false,
    })
    await page.getByTestId('network-editor-enter-edit').click()
    ...
    const atomicBox = await locatorClientRect(atomicActivity)
    ...
    expect(atomicBox.x).toBeGreaterThanOrEqual(virtualContainerBox.x - 2)
    expect(virtualBox.x + virtualBox.width).toBeLessThanOrEqual(virtualContainerBox.x + virtualContainerBox.width + 2)
    expect(atomicBox.x + atomicBox.width).toBeLessThanOrEqual(virtualContainerBox.x + virtualContainerBox.width + 2)
  })
