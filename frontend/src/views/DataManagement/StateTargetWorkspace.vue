<template>
  <div class="state-workspace">
    <div class="workspace-toolbar">
      <div>
        <h2>状态目标</h2>
        <p>用多层状态树维护目标包和原子状态，底层状态自动作为原子状态参与展开。</p>
      </div>
    ...
    <div v-else class="workspace-grid">
      ...
      <section class="panel">
        ...
        <el-alert
          ...
          title="原子状态绑定一个状态维度的二元取值；维度在“状态维度”页统一维护。"
          type="success"
          :closable="false"
          class="risk-alert"
        />
    ...
    <el-drawer v-model="drawerVisible" :title="drawerTitle" size="480px">
      <el-form :model="form" label-width="112px" @submit.prevent>
        ...
        <template v-if="form.state_kind !== 'aggregate'">
          ...
          <el-form-item label="状态维度" required>
            <el-select
              v-model="form.dimension_template_key"
              placeholder="选择已配置的状态维度"
              filterable
...
function normalizeCodeToken(value, fallback) {
  ...
  flushAscii()
  return parts.join('_').replace(/_+/g, '_').replace(/^_+|_+$/g, '') || fallback
}
...
async function save() {
  ...
  if (isAtomic && targetOptions.value.length !== 2) {
    return ElMessage.warning('状态维度需要先配置两个二元取值')
  }
  if (isAtomic && !targetOptions.value.includes(targetValue)) {
    return ElMessage.warning('目标取值必须从该状态维度的二元取值中选择')
  }
  saving.value = true
  try {
    if (isAtomic) {
