<template>
  <div class="dimension-page">
    <div class="page-toolbar">
      <div>
        <h2>通用状态维度</h2>
        <p>按设备类型维护通用二元模板，例如模块安装、管线连接、调测验证。</p>
      </div>
    ...
    <el-table v-else v-loading="loading" :data="dimensions" border size="small" class="dimension-table">
      <el-table-column prop="feature_name" label="通用状态维度" min-width="180">
    ...
    <el-drawer v-model="drawerVisible" :title="editId ? '编辑通用状态维度' : '新建通用状态维度'" size="440px">
...
async function remove(row) {
  ...
  await ElMessageBox.confirm('确认删除这个业务状态维度？', '确认', { type: 'warning' })
  await deleteFeatureDef(row.id)
  ElMessage.success('业务状态维度已删除')
  await loadAll()
}
