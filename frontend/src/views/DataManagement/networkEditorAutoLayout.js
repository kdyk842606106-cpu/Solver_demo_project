...
function normalizeRelationEdges(edges, stateNodeById, activityNodeById, relayIds) {
  ...
  for (const edge of edges || []) {
    if (edge?.aggregate || edge?.isCollapsedInternalProxy) continue
...
function assignRelayLayoutParents(rawParentById, parentById, items, edges) {
  for (const item of items.values()) {
    ...
    const commonParent = deepestCommonLayoutParent(connectedIds, parentById, items)
...
function nestedLayoutOptions(item, direction) {
  ...
  return {
    ...
    'elk.portConstraints': 'FIXED_SIDE',
    'elk.layered.considerModelOrder.strategy': 'NODES_AND_EDGES',
    'elk.spacing.nodeNode': direction === 'DOWN' ? '18' : '28',
    'elk.layered.spacing.nodeNodeBetweenLayers': direction === 'DOWN' ? '24' : '64',
    'elk.layered.spacing.edgeNodeBetweenLayers': '16',
    'elk.layered.spacing.edgeEdgeBetweenLayers': '10',
    'elk.layered.mergeEdges': 'false',
    'elk.separateConnectedComponents': 'true',
