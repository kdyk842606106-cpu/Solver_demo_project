        ...
        >
          ...
          <div class="resource-section">
            ...
            <el-tree
              ...
              data-testid="network-editor-activity-resource-tree"
              node-key="id"
              default-expand-all
              highlight-current
              :expand-on-click-node="false"
          ...
          >
            <NetworkEditorX6Canvas
              ...
              :expanded-state-graph-ids="Array.from(expandedStateGraphIds)"
              :activity-scope-ids="selectedActivityScopeIds"
          ...
          <div
            v-if="isStateTransitionView && selectedStateGraphNode"
            class="detail-block"
            data-testid="network-editor-state-transition-detail"
            :data-display-state-graph-id="selectedStateTransitionDetail?.displayStateGraphId || ''"
            :data-canonical-state-id="selectedStateTransitionDetail?.canonicalStateId || ''"
            :data-source-status="selectedStateTransitionDetail?.sourceStatus || 'committed'"
          >
...
import {
  STATE_TRANSITION_FALLBACK_COLUMN_STEP,
  STATE_TRANSITION_FALLBACK_ROW_STEP,
} from './networkEditorLayoutMetrics'
import { buildStateTransitionProjection } from './networkEditorStateTransitionProjection'
...
const expandedStateGraphIds = ref(new Set())
const activeExpandedStateGraphId = ref('')
const selectedActivityScopeIds = ref([])
const stateDepth = ref(1)
const activityDepth = ref(1)
...
const stateTransitionGraph = ref(null)
...
const stateTransitionLayoutCache = new Map()
...
const transitionLayoutColumnGap = STATE_TRANSITION_FALLBACK_COLUMN_STEP
const transitionLayoutRowGap = STATE_TRANSITION_FALLBACK_ROW_STEP
...
let impactDebounceResolve = null
...
const stateTransitionProjectionGraph = computed(() =>
  isStateTransitionCanvas.value && stateTransitionGraph.value ? stateTransitionGraph.value : graph.value,
)
const baseStateTransitionProjectionStateNodes = computed(() =>
  (stateTransitionProjectionGraph.value?.state_nodes || []).filter((node) => !stateGraphNodeDeleted(node)),
)
const stateTransitionProjectionStateNodes = computed(() =>
  uniqueGraphNodesById([
    ...baseStateTransitionProjectionStateNodes.value,
    ...draftStateGraphNodes.value,
  ]),
)
const stateTransitionStagedStateNodes = computed(() => buildStagedStateTransitionNodes(
  stateTransitionProjectionStateNodes.value,
))
const stateTransitionProjectionActivityNodes = computed(() =>
  uniqueGraphNodesById([
    ...allAtomicActivityGraphNodes.value,
    ...(stateTransitionProjectionGraph.value?.activity_nodes || []).filter((node) => !activityGraphNodeDeleted(node)),
    ...visibleActivityNodes.value,
  ]),
)
const stateTransitionProjectionActivityById = computed(() => {
  const result = new Map()
  for (const item of stateTransitionProjectionActivityNodes.value) {
    if (item?.id) result.set(item.id, item)
  }
  return result
})
const stateTransitionBaseEdges = computed(() =>
  (stateTransitionProjectionGraph.value?.edges || []).filter((edge) =>
    (!edge.binding_id || !deletedBindingIdSet.value.has(Number(edge.binding_id))) &&
    !edgeEndpointDeleted(edge),
  ),
)
const stateTransitionEdges = computed(() => {
  ...
  const edges = uniqueEdgesById([
    ...stateTransitionBaseEdges.value,
    ...draftBindingEdges.value,
  ])
  ...
  for (const binding of bindings.value) {
    ...
    const bindingKey = edge ? bindingEdgeDedupeKey({ ...edge, binding_id: binding.id }) : null
...
const stateTransitionBackboneEdges = computed(() => stateTransitionProjection.value.backboneEdges || [])
...
function uniqueGraphNodesById(nodes) {
  const seen = new Set()
  const result = []
...
function bindingEdgeDedupeKey(edge) {
...
const stateTransitionProjection = computed(() => buildStateTransitionProjection({
  ...
  renderedStateNodes: x6RawVisibleStateNodes.value,
  expandedStateGraphIds: expandedStateContainerIdsForAutoLayout(x6RawVisibleStateNodes.value, { useProjection: false }),
  collapsedStateContainerKeys: Array.from(collapsedStateContainerKeys.value),
  activityByGraphId: isStateTransitionCanvas.value ? stateTransitionProjectionActivityById.value : graphActivityById.value,
  bindingById: bindingById.value,
  fallbackActivityGraphNode,
  fallbackStateGraphNode,
  nodeLabel,
  nodeLabelForGraphId,
  isAtomicStateNode,
}))
const stateTransitionRealizersByStateId = computed(() => stateTransitionProjection.value.realizersByStateId)
const stateTransitionOutputsByActivityId = computed(() => stateTransitionProjection.value.outputsByActivityId)
const stateTransitionPreconditionsByActivityId = computed(() => stateTransitionProjection.value.preconditionsByActivityId)
const stateTransitionConsumersByStateId = computed(() => stateTransitionProjection.value.consumersByStateId)
const stateTransitionByStateId = computed(() => stateTransitionProjection.value.transitionsByStateId)
const stateTransitionWarningsByStateId = computed(() => stateTransitionProjection.value.warningsByStateId)
const canonicalStateIdByGraphId = computed(() => stateTransitionProjection.value.canonicalStateIdByGraphId)
const visibleStateGraphIdByCanonicalId = computed(() => stateTransitionProjection.value.visibleStateGraphIdByCanonicalId)
const stateTransitionGroupsByGraphId = computed(() => stateTransitionProjection.value.groupsByGraphId || new Map())
const stateTransitionVisibleEdges = computed(() => stateTransitionProjection.value.visibleEdges || [])
const stateTransitionDetailsByStateId = computed(() => stateTransitionProjection.value.detailsByStateId || new Map())
const selectedStateTransition = computed(() =>
  selectedStateTransitionDetail.value?.transition || null,
)
const selectedStateTransitionDetail = computed(() =>
  selectedStateId.value ? stateTransitionDetailsByStateId.value.get(stateNodeKey(selectedStateId.value)) || null : null,
)
const x6RawVisibleStateNodes = computed(() =>
  (isStateTransitionCanvas.value ? stateTransitionStagedStateNodes.value : visibleStateNodes.value)
    .filter(isStateVisibleInX6)
    .map(nodeWithDraftLayout)
    .map(nodeWithDraftContainer),
)
const x6BaseVisibleStateNodes = computed(() =>
  x6RawVisibleStateNodes.value
    .map(nodeWithStateTransition),
)
const stateTransitionRelayGroups = computed(() => stateTransitionProjection.value.relayGroups)
const stateTransitionLayoutSignatureValue = computed(() => stateTransitionLayoutSignature())
const stateTransitionVisualPlan = computed(() => {
  const fallback = buildStateTransitionVisualPlan(
    x6BaseVisibleStateNodes.value,
    stateTransitionRelayGroups.value,
    stateTransitionVisibleEdges.value,
  )
  const autoLayout = stateTransitionAutoLayout.value
  if (
    !isStateTransitionCanvas.value ||
    !autoLayout ||
    autoLayout.layoutSignature !== stateTransitionLayoutSignatureValue.value
  ) return fallback
  return {
    ...fallback,
...
const stateTransitionRelayNodes = computed(() => {
  ...
  return stateTransitionRelayGroups.value.map((group, index) => {
    ...
    return {
      ...
      parent_id: null,
      parent_graph_id: parentGraphId || null,
      ...
      metadata_json: {
        ...
        _network_editor_transition_relay: {
          ...
          parentGraphId: parentGraphId || null,
        },
...
const stagedActivityTree = computed(() => stageActivityResourceTree(activityTree.value))
...
const selectedTransitionRealizers = computed(() => selectedStateTransitionDetail.value?.realizerActivities || [])
const selectedTransitionPreconditions = computed(() => selectedStateTransitionDetail.value?.preconditions || [])
...
function isAtomicStateLibraryObject(node) {
  if (node?.state_kind === 'aggregate') return false
...
function activityResourceDepthForChild(child, parentDepth, rootIsTopLevel) {
  if (rootIsTopLevel && parentDepth === 1 && child?.resource_type === 'atomic_activity') return parentDepth + 2
  return parentDepth + 1
}

function stageActivityResourceBranch(node, maxDepth, rootIsTopLevel, depth = 1) {
  const children = (node.children || [])
    .map((child) => {
      const childDepth = activityResourceDepthForChild(child, depth, rootIsTopLevel)
      if (maxDepth > 0 && childDepth > maxDepth) return null
      return stageActivityResourceBranch(child, maxDepth, rootIsTopLevel, childDepth)
    })
    .filter(Boolean)
  return {
    ...node,
...
function indexActivityResourceNodes(items, index = new Map()) {
  for (const item of items || []) {
    if (item?.resource_type !== 'atomic_activity') index.set(String(item.id), item)
    indexActivityResourceNodes(item.children || [], index)
  }
  return index
}

function stageActivityResourceTree(tree) {
  const maxDepth = Number(activityDepth.value) || 0
  if (maxDepth <= 0) return tree
  const nodeIndex = indexActivityResourceNodes(tree)
  const roots = selectedActivityScopeIds.value.length
    ? selectedActivityScopeIds.value
        .map((rootId) => nodeIndex.get(String(rootId)))
        .filter(Boolean)
    : tree
  return roots.map((root) => {
    const rootIsTopLevel = root?.parent_id === null || root?.parent_id === undefined || root?.parent_id === ''
    return stageActivityResourceBranch(root, Math.max(1, maxDepth), rootIsTopLevel)
  })
}
...
function handleX6ProxyEdgeClick(payload) {
  ...
  const endpoints = [
    edge.proxyTargetId || edge.target_id,
    edge.proxySourceId || edge.source_id,
  ].filter(Boolean)
  const preferred = endpoints.find((id) => String(id).startsWith('state_node:')) || endpoints[0]
...
async function handleX6ProxyEdgeDoubleClick(payload) {
  const edge = payload?.edge
  if (!edge?.isCollapsedProxy) return
  if (edge.isPackageTransitionProxy) {
    await expandProxyEndpoint(edge.proxyTargetId || edge.target_id)
    return
  }
...
function handleX6LayoutChange(payload) {
  ...
  stateTransitionLayoutCache.clear()
...
function expandedStateContainerIdsForAutoLayout(nodes = x6BaseVisibleStateNodes.value, { useProjection = true } = {}) {
  if (isStateTransitionCanvas.value && expandedStateGraphIds.value.size) {
    const visibleIds = new Set((nodes || []).map((node) => String(node.id || '')).filter(Boolean))
    return [...expandedStateGraphIds.value].filter((id) => visibleIds.has(String(id)))
  }
  if (useProjection && isStateTransitionCanvas.value && stateTransitionGroupsByGraphId.value?.size) {
    return Array.from(stateTransitionGroupsByGraphId.value.values())
      .filter((group) => group.expanded && group.childGraphIds?.length)
      .map((group) => group.graphId)
  }
  return (nodes || [])
...
function isExpandedStateContainerForAutoLayout(node, nodes) {
  ...
  if (Number(stateDepth.value) === 0) return true
  const rootIds = selectedStateRootIds.value.map((id) => String(id || '')).filter(Boolean)
  if (!rootIds.length) return false
  const maxDepth = Math.max(1, Number(stateDepth.value) || 1)
  return rootIds.some((rootId) => {
    if (String(node.state_node_id || '') === rootId) return maxDepth > 1
    const scopeNode = (nodes || []).find((item) => String(item.state_node_id || '') === rootId)
...
function isExpandedActivityContainerForAutoLayout(node, nodes) {
  ...
  if (!hasVisibleChild) return false
  if (Number(activityDepth.value) === 0) return true
  const rootIds = selectedActivityScopeIds.value.map((id) => String(id || '')).filter(Boolean)
  if (!rootIds.length) return false
  const maxDepth = Math.max(1, Number(activityDepth.value) || 1)
  return rootIds.some((rootId) => {
    if (String(node.activity_node_id || '') === rootId) return maxDepth > 1
    const scopeNode = (nodes || []).find((item) => String(item.activity_node_id || '') === rootId)
...
function stateTransitionLayoutSignature() {
  const stateIds = x6BaseVisibleStateNodes.value.map((node) => String(node.id || '')).filter(Boolean).sort()
  const relayIds = stateTransitionRelayGroups.value.map((group) => String(group.relayId || '')).filter(Boolean).sort()
  const edgeIds = stateTransitionVisibleEdges.value.map((edge) => String(edge.id || '')).filter(Boolean).sort()
  const expandedIds = [...expandedStateGraphIds.value].map(String).sort()
  return JSON.stringify({
    states: stateIds,
    relays: relayIds,
    edges: edgeIds,
    expanded: expandedIds,
    active: String(activeExpandedStateGraphId.value || ''),
    focus: selectedStateRootIds.value.map(String).sort(),
    depth: Number(stateDepth.value),
  })
}

function activeStateTransitionLayoutScopeId() {
  const active = String(activeExpandedStateGraphId.value || '')
  if (active && expandedStateGraphIds.value.has(active)) return active
  return ''
}

function stateGraphIdsWithinScope(scopeId) {
  const ids = new Set([String(scopeId || '')])
  for (const id of stateGraphDescendantIds(scopeId)) ids.add(String(id))
  return ids
}

function relayIdsOwnedByStateScope(scopeStateIds) {
  const relayIds = new Set()
  for (const group of stateTransitionRelayGroups.value || []) {
    const outputs = (group.outputStateIds || []).map(String).filter(Boolean)
    if (outputs.length && outputs.every((id) => scopeStateIds.has(id))) relayIds.add(String(group.relayId))
  }
  return relayIds
}

function shiftedLayoutRoute(route, dx, dy) {
  if (!route) return route
  const shift = (point) => ({ x: Math.round(Number(point.x || 0) + dx), y: Math.round(Number(point.y || 0) + dy) })
  return {
    ...route,
    points: (route.points || []).map(shift),
    vertices: (route.vertices || []).map(shift),
  }
}

function normalizeStateTransitionPlanForActiveScope(plan, signature = stateTransitionLayoutSignatureValue.value) {
  if (!plan) return null
  const scopeId = activeStateTransitionLayoutScopeId()
  if (!scopeId) return { ...plan, layoutSignature: signature, layoutScopeGraphId: '' }
  const scopeNode = x6BaseVisibleStateNodes.value.find((node) => String(node.id || '') === scopeId)
  const plannedAnchor = plan.statePositions?.get(scopeId)
  if (!scopeNode || !plannedAnchor) return { ...plan, layoutSignature: signature, layoutScopeGraphId: scopeId }
  const currentAnchor = nodePosition(
    scopeNode,
    Math.max(0, x6BaseVisibleStateNodes.value.findIndex((node) => String(node.id || '') === scopeId)),
    'state',
  )
  const dx = Math.round(Number(currentAnchor.x || 0) - Number(plannedAnchor.x || 0))
  const dy = Math.round(Number(currentAnchor.y || 0) - Number(plannedAnchor.y || 0))
  const scopeStateIds = stateGraphIdsWithinScope(scopeId)
  const ownedRelayIds = relayIdsOwnedByStateScope(scopeStateIds)
  const statePositions = new Map()
  for (const [id, position] of plan.statePositions || []) {
    if (!scopeStateIds.has(String(id)) || String(id) === scopeId) continue
    statePositions.set(String(id), {
      x: Math.round(position.x + dx),
      y: Math.round(position.y + dy),
    })
  }
  const relayPositions = new Map()
  for (const [id, position] of plan.relayPositions || []) {
    if (!ownedRelayIds.has(String(id))) continue
    relayPositions.set(String(id), { x: Math.round(position.x + dx), y: Math.round(position.y + dy) })
  }
  const ownedEndpointIds = new Set([...scopeStateIds, ...ownedRelayIds])
  const edgeById = new Map(x6ResolvedEdges.value.map((edge) => [String(edge.id || ''), edge]))
  const edgeRoutes = new Map()
  for (const [id, route] of plan.edgeRoutes || []) {
    const edge = edgeById.get(String(id))
    if (!edge || !ownedEndpointIds.has(String(edge.source_id)) || !ownedEndpointIds.has(String(edge.target_id))) continue
    edgeRoutes.set(String(id), shiftedLayoutRoute(route, dx, dy))
  }
  const containerSizes = new Map()
  for (const [id, size] of plan.containerSizes || []) {
    if (scopeStateIds.has(String(id))) containerSizes.set(String(id), size)
  }
  return {
    ...plan,
    statePositions,
    ...
    layoutScopeGraphId: scopeId,
  }
}

async function buildCurrentStateTransitionLayoutPlan() {
  const scopeId = activeStateTransitionLayoutScopeId()
  const scopeStateIds = scopeId ? stateGraphIdsWithinScope(scopeId) : null
  const ownedRelayIds = scopeStateIds ? relayIdsOwnedByStateScope(scopeStateIds) : null
  const stateNodes = scopeStateIds
    ? x6BaseVisibleStateNodes.value.filter((node) => scopeStateIds.has(String(node.id || '')))
    : x6BaseVisibleStateNodes.value
  const relayNodes = ownedRelayIds
    ? stateTransitionRelayNodes.value.filter((node) => ownedRelayIds.has(String(node.id || '')))
    : stateTransitionRelayNodes.value
  const endpointIds = scopeStateIds
    ? new Set([...scopeStateIds, ...(ownedRelayIds || [])])
    : null
  const edges = endpointIds
    ? x6ResolvedEdges.value.filter((edge) =>
        endpointIds.has(String(edge.source_id || '')) && endpointIds.has(String(edge.target_id || '')),
      )
    : x6ResolvedEdges.value
  const raw = await layoutNestedContainerGraph({
    stateNodes,
    activityNodes: relayNodes,
    relayNodes,
    edges,
  ...
  return normalizeStateTransitionPlanForActiveScope(raw)
}

async function refreshStateTransitionLocalLayout() {
  if (!isStateTransitionCanvas.value) return
  const signature = stateTransitionLayoutSignatureValue.value
  const cached = stateTransitionLayoutCache.get(signature)
  if (cached) {
    stateTransitionAutoLayout.value = cached
    return
  }
  if (!activeStateTransitionLayoutScopeId()) {
    stateTransitionAutoLayout.value = null
    return
  }
  const requestId = graphRequestSequence
  try {
    const plan = await buildCurrentStateTransitionLayoutPlan()
    if (requestId !== graphRequestSequence || signature !== stateTransitionLayoutSignatureValue.value) return
    stateTransitionAutoLayout.value = plan
    stateTransitionLayoutCache.set(signature, plan)
  } catch (error) {
    console.warn('Network Editor local state-transition layout failed; using staged fallback.', error)
    stateTransitionAutoLayout.value = null
  }
}
...
async function autoArrangeStateTransitionCanvas() {
  ...
  try {
    plan = await buildCurrentStateTransitionLayoutPlan()
    if (!plan?.diagnostics?.nodeCount) {
      plan = buildStateTransitionVisualPlan(
        x6BaseVisibleStateNodes.value,
        stateTransitionRelayGroups.value,
        stateTransitionVisibleEdges.value,
      )
      plan = normalizeStateTransitionPlanForActiveScope(plan)
      usedFallback = true
    }
    stateTransitionAutoLayout.value = plan
    stateTransitionLayoutCache.set(stateTransitionLayoutSignatureValue.value, plan)
  } catch (error) {
...
function buildX6CollapsedRelationBadges(edges) {
  ...
  for (const edge of edges) {
    ...
    if (edge.isPackageTransitionProxy) {
      const sourceBadge = ensure(edge.source_id)
      const targetBadge = ensure(edge.target_id)
      if (sourceBadge) sourceBadge.output += count
      if (targetBadge) targetBadge.input += count
      continue
    }
...
function buildStateTransitionVisualPlan(stateNodes = [], relayGroups = [], projectionEdges = []) {
  ...
  const layoutOnlyProxyGroups = (projectionEdges || [])
    .filter((edge) =>
      edge?.isPackageTransitionProxy &&
      edge.source_id !== edge.target_id &&
      stateNodeById.has(String(edge.source_id)) &&
      stateNodeById.has(String(edge.target_id)),
    )
    .map((edge) => ({
      relayId: `layout-proxy-relay:${edge.id}`,
      activityId: edge.id,
      inputStateIds: [edge.source_id],
      outputStateIds: [edge.target_id],
      layoutOnly: true,
    }))
  const usableGroups = [...(relayGroups || []), ...layoutOnlyProxyGroups].filter((group) => {
  ...
  for (const group of usableGroups.filter((item) => !item.layoutOnly)) {
...
function flowEdgeGraphIds(edge) {
  const ids = [
    ...
    ...(edge.projectionGroupIds || []),
    ...(edge.underlyingTransitionIds || []),
    ...(edge.underlyingActivityIds || []),
    edge.hiddenSourceId,
    edge.hiddenTargetId,
...
function edgeDisplayLabel(edge) {
  ...
  if (edge.packageCoverageLabel) return edge.packageCoverageLabel
...
function edgeTitle(edge) {
  ...
  if (isTransitionRelayEdge(edge)) {
    if (edge.type === 'STATE_TO_ACTIVITY') {
      const title = `前置条件：${nodeLabelForGraphId(edge.source_id)} -> ${edge.flowRealizerLabel || nodeLabelForGraphId(edge.flowActivityId)}`
      return edge.packageCoverageTitle ? `${title} · ${edge.packageCoverageTitle}` : title
    }
    const title = `达成结果：${edge.flowRealizerLabel || nodeLabelForGraphId(edge.flowActivityId)} -> ${nodeLabelForGraphId(edge.target_id)}`
    return edge.packageCoverageTitle ? `${title} · ${edge.packageCoverageTitle}` : title
  }
...
function shouldLoadStateTransitionProjectionGraph() {
  return isStateTransitionCanvas.value
}

function stateTransitionProjectionPayload(basePayload = graphPayload()) {
  return {
    ...basePayload,
    state_root_ids: [],
...
function isTopLevelActivityNode(node) {
  return !!node?.activity_node_id &&
    (node.parent_id === null || node.parent_id === undefined || node.parent_id === '')
}

function stateGraphParentId(node, nodes = stateTransitionProjectionStateNodes.value) {
  if (!node?.id) return ''
  const byId = new Map((nodes || []).map((item) => [String(item.id || ''), item]))
  for (const graphId of [node.primary_parent_graph_id, node.parent_graph_id, node.reference_parent_graph_id]) {
    const key = String(graphId || '')
    if (key && key !== String(node.id) && byId.has(key)) return key
  }
  const parentIds = [node.parent_id, ...(node.reference_parent_ids || [])]
    .map((id) => String(id || ''))
    .filter(Boolean)
  for (const parentId of parentIds) {
    const exact = `state_node:${parentId}`
    if (exact !== String(node.id) && byId.has(exact)) return exact
    const candidate = (nodes || []).find((item) =>
      String(item.id || '') !== String(node.id) &&
      String(item.state_node_id || '') === parentId &&
      Number(item.level || 0) < Number(node.level || Infinity),
    )
...
function buildStateGraphHierarchy(nodes = stateTransitionProjectionStateNodes.value) {
  const byId = new Map((nodes || []).map((node) => [String(node.id || ''), node]).filter(([id]) => id))
  const parentById = new Map()
  const childrenByParent = new Map()
  for (const [id, node] of byId.entries()) {
    const parentId = stateGraphParentId(node, nodes)
...
function buildStagedStateTransitionNodes(nodes = []) {
  const hierarchy = buildStateGraphHierarchy(nodes)
  const focusIds = new Set(selectedStateRootIds.value.map((id) => String(id || '')).filter(Boolean))
  let rootIds = Array.from(hierarchy.byId.keys()).filter((id) => !hierarchy.parentById.get(id))
  if (focusIds.size) {
    const focused = Array.from(hierarchy.byId.entries())
      .filter(([, node]) => focusIds.has(String(node.state_node_id || '')))
      .map(([id]) => id)
    if (focused.length) rootIds = focused
  }
  const visible = new Set()
  const queue = rootIds.map((id) => ({ id, depth: 1 }))
  while (queue.length) {
    const current = queue.shift()
    if (!current?.id || visible.has(current.id)) continue
    visible.add(current.id)
    const locallyExpanded = expandedStateGraphIds.value.has(current.id)
    const focusExpanded = focusIds.size && (Number(stateDepth.value) === 0 || current.depth < Math.max(1, Number(stateDepth.value) || 1))
    if (!locallyExpanded && !focusExpanded) continue
    for (const childId of hierarchy.childrenByParent.get(current.id) || []) {
      queue.push({ id: childId, depth: current.depth + 1 })
    }
  }
  return Array.from(visible).map((id) => hierarchy.byId.get(id)).filter(Boolean)
}

function stateGraphDescendantIds(rootGraphId, nodes = stateTransitionProjectionStateNodes.value) {
  const hierarchy = buildStateGraphHierarchy(nodes)
  const descendants = new Set()
  const queue = [...(hierarchy.childrenByParent.get(String(rootGraphId || '')) || [])]
  while (queue.length) {
    const id = queue.shift()
    if (!id || descendants.has(id)) continue
    descendants.add(id)
    queue.push(...(hierarchy.childrenByParent.get(id) || []))
  }
  return descendants
}

function nearestExpandedStateParentGraphId(graphId) {
  const hierarchy = buildStateGraphHierarchy()
  let parentId = hierarchy.parentById.get(String(graphId || '')) || ''
  while (parentId) {
    if (expandedStateGraphIds.value.has(parentId)) return parentId
    parentId = hierarchy.parentById.get(parentId) || ''
  }
  return ''
}

function expandedStateOwnerGraphId(graphId) {
  const key = String(graphId || '')
  if (expandedStateGraphIds.value.has(key)) return key
  return nearestExpandedStateParentGraphId(key)
}

function relayExpandedStateOwnerGraphId(group) {
  const hierarchy = buildStateGraphHierarchy()
  const chains = (group?.outputStateIds || [])
    .map((graphId) => {
      const chain = []
      let current = String(graphId || '')
      while (current) {
        if (expandedStateGraphIds.value.has(current)) chain.push(current)
  ...
  return chains[0].find((id) => otherChains.every((chain) => chain.has(id))) || ''
}

function resetLocalStateTransitionExpansion() {
  expandedStateGraphIds.value = new Set()
  activeExpandedStateGraphId.value = ''
  stateTransitionAutoLayout.value = null
  stateTransitionLayoutCache.clear()
}

function pruneLocalStateTransitionExpansion() {
  const valid = new Set(stateTransitionProjectionStateNodes.value.map((node) => String(node.id || '')).filter(Boolean))
  const next = new Set([...expandedStateGraphIds.value].filter((id) => valid.has(String(id))))
  if (next.size !== expandedStateGraphIds.value.size) expandedStateGraphIds.value = next
  if (activeExpandedStateGraphId.value && !next.has(activeExpandedStateGraphId.value)) {
    activeExpandedStateGraphId.value = ''
  }
}

async function toggleLocalStateTransitionExpansion(node) {
  const key = String(node?.id || '')
  if (!key) return
  const next = new Set(expandedStateGraphIds.value)
  if (next.has(key)) {
    next.delete(key)
    for (const descendantId of stateGraphDescendantIds(key)) next.delete(descendantId)
    if (Number(stateDepth.value) === 0) stateDepth.value = 2
    expandedStateGraphIds.value = next
    activeExpandedStateGraphId.value = nearestExpandedStateParentGraphId(key)
  } else {
    next.add(key)
    expandedStateGraphIds.value = next
    activeExpandedStateGraphId.value = key
  }
  stateTransitionAutoLayout.value = null
  await nextTick()
  await refreshStateTransitionLocalLayout()
}

function isStateVisibleInX6(node) {
  if (!node?.state_node_id) return false
  // Atomic state definitions are library records. Only their package-scoped
  // reference instances belong on the canvas, including in diagnostic mode.
  if (isAtomicStateLibraryObject(node)) return false
  if (!isStateTransitionCanvas.value && isInsideCollapsedStateContainer(node)) return false
...
function isActivityVisibleInX6(node) {
  ...
  if (activityDepth.value === 0) return true
  const maxDepth = Math.max(1, Number(activityDepth.value) || 1)
  if (!selectedActivityScopeIds.value.length) return isTopLevelActivityNode(node)
  return selectedActivityScopeIds.value.some((rootId) => {
    const relativeDepth = activityRelativeDepthFromRoot(node, rootId)
    return Number.isFinite(relativeDepth) && relativeDepth >= 1 && relativeDepth <= maxDepth
  })
}
...
function isInsideCollapsedActivityContainer(node) {
  ...
  for (const key of collapsedActivityContainerKeys.value) {
    ...
    if (
      container?.activity_node_id &&
      selectedActivityScopeIds.value.some((id) => String(id) === String(container.activity_node_id)) &&
      (Number(activityDepth.value) === 0 || Number(activityDepth.value) > 1)
    ) {
      continue
    }
...
function expandStateContainerForDraftChild(parentId) {
  if (!parentId) return
  const candidates = isStateTransitionCanvas.value ? stateTransitionProjectionStateNodes.value : visibleStateNodes.value
  const parentGraphNode = candidates.find((node) =>
    String(node.state_node_id || '') === String(parentId),
  )
  if (isStateTransitionCanvas.value && parentGraphNode?.id) {
    expandedStateGraphIds.value = new Set([...expandedStateGraphIds.value, String(parentGraphNode.id)])
    activeExpandedStateGraphId.value = String(parentGraphNode.id)
    stateTransitionAutoLayout.value = null
    return
  }
  const parentRootId = parentGraphNode?.state_node_id || parentId
  selectedStateRootIds.value = [parentRootId]
  stateDepth.value = 2
...
function isGraphStateExpanded(node) {
  ...
  return selectedStateRootIds.value.some((selectedId) => {
    const scopeId = String(selectedId)
    if (scopeId === String(node.state_node_id)) return stateDepth.value === 0 || stateDepth.value > 1
    const scopeNode = visibleStateNodes.value.find((item) => String(item.state_node_id || '') === scopeId)
    return !!scopeNode && statePathContains(scopeNode, node.state_node_id)
  })
}

function isGraphActivityExpanded(node) {
  if (!node?.activity_node_id || selectedActivityScopeIds.value.length !== 1) return false
  const scopeId = String(selectedActivityScopeIds.value[0])
  if (scopeId === String(node.activity_node_id)) return activityDepth.value === 0 || activityDepth.value > 1
  const scopeNode = visibleActivityNodes.value.find((item) => String(item.activity_node_id || '') === scopeId)
  return !!scopeNode && activityPathContains(scopeNode, node.activity_node_id)
}

async function toggleGraphStateExpansion(node) {
  if (!node?.state_node_id) return
  if (isStateTransitionCanvas.value) {
    closeContextMenu()
    selectedStateId.value = node.state_node_id
    selectionFocus.value = 'state'
    bindingForm.value.state_node_id = node.state_node_id
    resetBindingCoverageSelection()
    await toggleLocalStateTransitionExpansion(node)
    return
  }
  ...
  if (wasExpanded) {
    ...
    stateDepth.value = selectedStateRootIds.value.length ? 2 : 1
...
async function toggleGraphActivityExpansion(node) {
  ...
  resetCollapsedActivityContainers()
  if (wasExpanded) {
    if (Number(node.level || 0) > 1 && node.parent_id) {
      selectedActivityScopeIds.value = [node.parent_id]
      activityDepth.value = 2
    } else {
      selectedActivityScopeIds.value = []
      activityDepth.value = 1
    }
  } else {
...
async function collapseCurrentSelection() {
  ...
  if (selectedActivityScopeIds.value.length) {
    const node = activityNodeById.value.get(selectedActivityScopeIds.value[0])
    if (node?.parent_id && Number(node.level || 0) > 1) {
      selectedActivityScopeIds.value = [node.parent_id]
      activityDepth.value = 2
    } else {
      selectedActivityScopeIds.value = []
...
async function expandCurrentSelectionAll() {
  ...
  await reloadGraph()
  if (isStateTransitionCanvas.value && selectedStateRootIds.value.length) {
    expandedStateGraphIds.value = new Set(
      stateTransitionProjectionStateNodes.value
...
function selectGraphState(node, event = null) {
  ...
  resetBindingCoverageSelection()
  if (isStateTransitionCanvas.value) {
    const ownerId = expandedStateOwnerGraphId(node.id)
    if (ownerId && ownerId !== activeExpandedStateGraphId.value) {
      activeExpandedStateGraphId.value = ownerId
      stateTransitionAutoLayout.value = null
      nextTick(() => refreshStateTransitionLocalLayout())
    }
  }
  ensureTransitionRulesForState(node.state_node_id)
  loadImpact()
...
async function revealIssueActivity(activityId) {
  ...
  if (activityId.startsWith('atomic_activity:')) {
    ...
    activityDepth.value = packageId ? 2 : 1
...
async function onTypeChange() {
  ...
  solverPrecheck.value = null
  impactResult.value = null
  stateTransitionGraph.value = null
  editorMode.value = 'preview'
  draftChanges.value = []
  draftSequence.value = 0
  draftBatchSequence.value = 0
...
async function reloadGraph({ preserveAutoLayout = false } = {}) {
  ...
  const requestId = graphRequestSequence + 1
  graphRequestSequence = requestId
  const requestMachineTypeId = machineTypeId.value
  const mainPayload = graphPayload()
  const projectionPayload = shouldLoadStateTransitionProjectionGraph()
    ? stateTransitionProjectionPayload(mainPayload)
    : null
  try {
    const [nextGraph, nextStateTransitionGraph] = await Promise.all([
      previewNetworkEditorGraph(requestMachineTypeId, mainPayload),
      projectionPayload
        ? previewNetworkEditorGraph(requestMachineTypeId, projectionPayload)
        : Promise.resolve(null),
    ])
    if (requestId !== graphRequestSequence || requestMachineTypeId !== machineTypeId.value) return
    graph.value = nextGraph
    stateTransitionGraph.value = projectionPayload ? nextStateTransitionGraph : null
    pruneLocalStateTransitionExpansion()
...
function clearImpactDebounce() {
  ...
  if (impactDebounceResolve) {
    const resolve = impactDebounceResolve
    impactDebounceResolve = null
    resolve(null)
  }
}
