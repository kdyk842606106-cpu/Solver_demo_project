...
const activityForm = reactive({ name: '', duration: 1, package_id: null, precondition_state_id: null, relation_role: 'transition', is_milestone: false })
...
function openActivityDialog() { Object.assign(activityForm, { name: '', duration: 1, package_id: null, precondition_state_id: null, relation_role: 'transition', is_milestone: false }); activityDialog.value = true }
function queueActivity() { if (!activityForm.name.trim()) return; if (!activityForm.precondition_state_id && !activityForm.is_milestone) return ElMessage.warning('非里程碑活动必须选择替换前置'); const ref = `draft:activity:${Date.now()}`; const preconditions = activityForm.precondition_state_id ? [{ state_id: activityForm.precondition_state_id, relation_role: activityForm.relation_role }] : []; addDraft({ operation: 'create_activity', client_ref: ref, payload: { name: activityForm.name.trim(), duration: activityForm.duration, preconditions, is_milestone: activityForm.is_milestone } }, `新建活动 ${activityForm.name}`); if (activityForm.package_id) addDraft({ operation: 'add_membership', payload: { package_id: activityForm.package_id, activity_id: ref } }, `加入活动包`); activityDialog.value = false }
